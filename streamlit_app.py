...
# Apply filters if clicked, else show full data
if apply:
    filtered_df = df[
...
# To avoid duplicated movies because of exploded genres
unique_movies_df = filtered_df.drop_duplicates(subset=['Movie Name'])

# Display filtered data
st.subheader("🎬 Filtered Movies")
st.dataframe(unique_movies_df)

# Top 10 Movies by Rating and Voting Counts
st.subheader("🏆 Top 10 Movies by Rating & Voting Counts")
top_movies = unique_movies_df.sort_values(by=['Rating', 'Voting Counts'], ascending=[False, False]).head(10)
...
fig1 = px.bar(genre_counts, x='Genre', y='Count', labels={'Genre': 'Genre', 'Count': 'Number of Movies'})
...
fig4 = px.histogram(unique_movies_df, x='Rating')
...
shortest = unique_movies_df.nsmallest(1, 'Duration')
longest = unique_movies_df.nlargest(1, 'Duration')
...
fig7 = px.scatter(unique_movies_df, x='Rating', y='Voting Counts', size='Duration', color='Genre', hover_data=['Movie Name'])
